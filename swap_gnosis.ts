...
// Safe executes in one batch: approve(s) if needed + Universal Router.execute(swap).
...
const amountIn = await usdcContract.balanceOf(SAFE_ADDRESS);
if (amountIn.isZero()) {
  throw new Error("[swap_gnosis.ts] Safe has no USDC balance to swap.");
}

const poolKey = {
  currency0: USDC_TOKEN.address,
  currency1: DAI_TOKEN.address,
  fee: 50,
  tickSpacing: 1,
  hooks: HOOKS_ZERO,
};

// Quote then apply slippage (debug_swap_EOA §8)
...
const needErc20Approve = erc20Allowance.lt(amountIn);
...
const needPermit2Approve =
  permit2Amount.lt(amountIn) || Number(permit2Expiration) <= now;
...
console.log("[LN:208][swap_gnosis.ts] Safe transaction executed (USDC -> DAI).");

const receipt = await txResponse.transactionResponse.wait();
console.log("[LN:202][swap_gnosis.ts] Transaction receipt.", receipt.transactionHash);
